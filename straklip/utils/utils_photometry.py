...
def KLIP_aperture_photometry_handler(DF,id,filter,data_label='',dq_label='',hdul=None,data=[],dqdata=None,ext=None,x=None,y=None,la_cr_remove=False,Python_origin=False,forcedSky=[],exptime=1,zpt=0,ezpt=0,thrpt=1,ethrpt=0,sat_thr=np.inf,radius_a=3,radius1=5,radius2=15,aptype='circular',df_label='mvs_tiles_df',df_ids='mvs_ids',kill_plots=False,noBGsub=False,grow_curves=False,ee_df=None,sigma=3,delta=3,gstep=0.01,p=5,r_in=6,r_min=6,r_max=15,multiply_by_exptime=False,multiply_by_gain=False,multiply_by_PAM=False,ROTA=None,PAV3=None,gain=None):
    ...
    if not kill_plots:
        fig,ax=plt.subplots(1,rows,figsize=(7*rows,5))
        if ROTA==None: ROTA=DF.mvs_targets_df.loc[DF.mvs_targets_df.mvs_ids==id,f'rota_{filter}'].values[0]
        if PAV3==None:PAV3=DF.mvs_targets_df.loc[DF.mvs_targets_df.mvs_ids==id,f'pav3_{filter}'].values[0]
...
def mvs_aperture_photometry(DF,filter,ee_dict,zpt_dict,fitsname=None,mvs_ids_list_in=[],data_label='',dq_label='',label='data',bpx_list=[],spx_list=[],la_cr_remove=False,cr_radius=1,radius_ap=10,radius_sky_inner=10,radius_sky_outer=15,sat_thr=np.inf,kill_plots=True,grow_curves=True,gstep=0.01,p=30,r_in=4,Python_origin=False,remove_candidate=False,multiply_by_exptime=False,multiply_by_gain=False,multiply_by_PAM=False,noBGsub=False,forceSky=False,path2savefile=None):
    ...
    for mvs_ids in mvs_ids_list:
        ...
        if len(mvs_ids_list_in)>1:
            phot.append([int(mvs_ids),float(ext),float(counts),float(ecounts),float(Nap),float(mag),float(emag),float(spx),float(bpx),float(radius),float(radius1),float(radius2),float(skym),float(esky),float(nSky),float(grow_corr)])
        else:
            phot=[int(mvs_ids),float(ext),float(counts),float(ecounts),float(Nap),float(mag),float(emag),float(spx),float(bpx),float(radius),float(radius1),float(radius2),float(skym),float(esky),float(nSky),float(grow_corr)]
        # except:
